...
const AIPrediction = () => {
    ...
    useEffect(() => {
        const fetchPredictions = async () => {
            try {
                // Use the local backend proxy to avoid CORS
                // Assuming the backend is running on the same host/port as defined in environment or default to localhost:5000
                const apiBaseUrl = import.meta.env.VITE_API_BASE_URL || 'http://localhost:5000';
                const response = await fetch(`${apiBaseUrl}/api/gold-forecast`);
                if (!response.ok) {
                    throw new Error('Failed to fetch predictions');
                }
                const data = await response.json();

                // Transform object { "YYYY-MM-DD": price } to array [{ date, price }]
                const formattedData = Object.entries(data).map(([date, price]) => ({
                    date,
    ...
    const convertPrice = (value) => (currency === 'USD' ? value : value * usdToBdtRate);
